    ...
    def __init__(self, master):
        ...
        def Tuple(event):
            try:
                ...
                self.admission_entry.insert(END, st[3])
        ...
        def Insert():
            if (len(self.admission.get()) != 0):
                Fee_Backend.insert(self.receipt.get(), self.name.get(), self.admission.get(), self.date.get(),
                                   self.branch.get(), self.sem.get(), self.total.get(), self.paid.get(),
                                   self.due.get())
                self.list.delete(0, END)
                self.list.insert(END, (self.receipt.get(), self.name.get(), self.admission.get(), self.date.get(),
                                       self.branch.get(), self.sem.get(), self.total.get(), self.paid.get(),
                                       self.due.get()))
        ...
        def Receipt():
            ...
            self.Display.insert(END, '\tDue Amount    :' + str(x3) + '\n')

            self.due.set(x3)

        def Search():
            self.list.delete(0, END)
            for row in Fee_Backend.search(self.receipt.get()):
        ...
        #==================================================Frames===================================================
        Main_Frame = Frame(self.master, bg='#45A29E')
        Main_Frame.grid()

        Title_Frame = LabelFrame(
            Main_Frame, width=1920, height=100, bg='#45A29E', relief='ridge', bd=15)
        Title_Frame.pack(side=TOP)

        self.lblTitle = Label(Title_Frame, font=('arial', 40, 'bold'), text='FEE MANAGEMENT SYSTEM',
                              bg='#45A29E', padx=13)
        self.lblTitle.grid(padx=600)

        Data_Frame = Frame(Main_Frame, width=1920, height=350,
                           bg='#45A29E', relief='ridge', bd=15)
        ...
        List_Frame = Frame(Main_Frame, width=1920, height=150,
                           bg='lightgreen', relief='ridge', bd=15)
        List_Frame.pack(side=TOP, padx=15)

        Button_Frame = Frame(Main_Frame, width=1920, height=80,
                             bg='lightgreen', relief='ridge', bd=15)
        Button_Frame.pack(side=TOP)

        #===================================================Labels================================================
        self.receipt_label = Label(Frame_1, text='Receipt No. : ', font=(
        ...
        self.admission_label = Label(Frame_1, text='Admission No. : ', font=(
            'arial', 14, 'bold'), bg='lightgreen')
        self.admission_label.grid(row=2, column=0, padx=15, sticky=W)
        ...
        self.due_label = Label(Frame_1, text='DUE AMOUNT : ', font=(
            'arial', 14, 'bold'), bg='lightgreen')
        self.due_label.grid(row=4, column=2, padx=5, sticky=W)

        #==================================================Entries=================================================
        self.var_1 = DoubleVar(Frame_1, value='70000')
        d1 = datetime.date.today()
        self.date.set(d1)

        self.receipt_entry = Entry(Frame_1, font=(
            'arial', 14), textvariable=self.receipt)
        ...
        self.admission_entry = Entry(Frame_1, font=(
            'arial', 14), textvariable=self.admission)
        self.admission_entry.grid(row=2, column=1, padx=15, pady=5)

        self.Date_entry = Entry(Frame_1, font=(
            'arial', 14), textvariable=self.date)
        self.Date_entry.grid(row=3, column=1, padx=15, pady=5)

        self.branch_entry = ttk.Combobox(Frame_1, values=(' ', 'BCA', 'BBA', 'B.Com', 'BJMC'),
